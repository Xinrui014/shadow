# finetune the sam model on the visha dataset
import os
...
def main():
    ...
    sam_model.train()
    print("Params: {}M".format(sum(p.numel() for p in sam_model.mask_decoder.parameters())/1e6))
    

    # set the optimizer and loss
    lr = 1e-4
    optimizer = torch.optim.Adam(sam_model.mask_decoder.parameters(), lr=lr, weight_decay=0)
    # loss_fn = torch.nn.MSELoss()
    loss_fn = torch.nn.BCEWithLogitsLoss()


    # load bounding boxes
    f = open("./sam_train.json", "r")
    content = f.read()
    meta = json.loads(content)
    bboxes = meta["Ball_ce1"]["00000099.png"]['bbox']
    bboxes = np.array(bboxes)
    sam_trans = ResizeLongestSide(sam_model.image_encoder.img_size)
    box = sam_trans.apply_boxes(bboxes, (1024, 1024))
    ...
    for epoch in range(num_epochs):
        ...
        for i, img_path in enumerate(img_all_pbar):
            ...
            gt_mask_resized = torch.from_numpy(np.resize(label, (1, 1, label.shape[0], label.shape[1]))).to(device)
            ...
            if i % 100 == 0:
                ...
                # image_save = cv2.resize(image_save, (256, 256))
        ...
        torch.save(sam_model.state_dict(), f"./checkpoints/chk_sam/{epoch}.pth")
